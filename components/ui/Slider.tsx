...
const onLoad = ({ rootId, infinite, gap }: Props) => {
  function init() {
    const root = document.getElementById(rootId);
    // @ts-expect-error Type 'typeof KeenSlider' is not assignable to type 'KeenSlider'.
    const slider = new KeenSlider(
      root?.querySelector<HTMLElement>("[data-slider]"),
      {
        loop: infinite,
        spacing: gap,
        slides: { perView: "auto" },
    ...
    for (let it = 0; it < (dots?.length ?? 0); it++) {
      dots?.item(it).addEventListener("click", () => slider.moveToIdx(it));
    }
